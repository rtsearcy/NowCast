# getRawMet_NCDC.py - Download raw meteorological data from National Climatic Data Center (NCDC).
# RTS - 3/21/2018

# Grabs hourly METAR data for airport stations along the coast, saves to file
# Parses raw data into met variables
# Note: This is a different data source for METAR data than what is used for implementation.
# Data matches up well, though.
...
# Use this folder for populating raw met data into the implementation data folder
...
for a in air_list:
    ...
    raw_skip = rs
    # Grab data from NCDC
    if raw_skip == 0 | len([x for x in os.listdir(outfolder_raw) if x.startswith(a.replace(' ', ''))]) == 0:
        ...
        try:
            ...
            raw_file = a.replace(' ', '_') + '_raw_METAR_data_' + sd_new.replace('-', '') + '_' \
                + ed_new.replace('-', '') + '.csv'
            df_raw.to_csv(outfolder_raw + raw_file)
        ...
        try:
            raw_file = [x for x in os.listdir(outfolder_raw) if x.startswith(a.replace(' ', '_'))][0]
    ...
    df_daily.to_csv(outfolder_daily + dailyfile)  # Save daily file
